...
export function format (
  mod :MD.Module, tree :T.RootTree, focus? :T.Path, opts :FormatOpts = {}
...
class Acc {
  ...
  appendTree (path :T.Path, tree :T.Tree) :this {
    ...
    if (tree instanceof T.EmptyTermTree || tree instanceof T.EmptyTypeTree) {
      // nothing (TODO?)

    } else if (tree instanceof T.TopTermDefTree) {
      // TODO: display quantifications (& constraints) of signature
      if (tree.body instanceof T.AbsTree) {
      ...
      this.appendAnnot(start, tree.treeType)
    ...
    //   this.appendTypeExprSpan(path, `${tpe.tag}${tpe.size}`)
      ...
      // this.appendSepSpan("]")
      this.appendAnnot(start, tree.treeType.kind)
  ...
  appendSymSpan (span :SymRuleSpan) {
    const start = this.lineWidth, {path, sym} = span
    this.appendSpan(span, path)
    this.appendAnnot(start, sym.kind === "term" ? sym.type : sym.type.kind)
  }
...
type KeyRule = {
  name :string
  chord :string
  pathPred :(root :T.RootTree, path :T.Path) => boolean
  apply :(root :T.RootTree, path :T.Path) => T.TreeEdit
  focusOp? :(root :T.RootTree, path :T.Path) => T.Path
}
...
const addArgType :KeyRule = {
  ...
  apply: (root, path) => {
    ...
    return typePath.selected(root) instanceof T.EmptyTermTree ?
...
const addInApp :KeyRule = {
  ...
  pathPred: (root, path) => {
    ...
    return selTree.kind === "lit" || (selTree.kind === "ref" && !selTree.treeType.isArrow)
  },
...
const addFunArgs :KeyRule = {
  ...
  pathPred: (root, path) => (!path.endsWith(root, "inapp", "fun") &&
                             ...
                             path.selectedTree(root).treeType.isArrow),
  apply: (root, path) => {
    const selTree = path.selectedTree(root)
    // otherwise we're sticking a fun somewhere new, so add holes for all args
    const edits = []
    const args = selTree.treeType.arity
...
function findBody (tree :T.Tree, path :T.Path) :T.Path {
  ...
  if (tree instanceof T.TopTermDefTree ||
...
abstract class SymRuleSpan extends RuleSpan {
  ...
  get tooltip () :string { return `${this.sym.type}` }
}
...
class TypeDefSpan extends SymRuleSpan {
  ...
  get tooltip () :string {
    const symType = this.tree.treeType
...
class TermExprSpan extends SymRuleSpan {
  ...
  getCompletions (text :string) :M.Completion[] {
    ...
    const selType = TP.hole // sel instanceof T.Tree ? sel.prototype : TP.hole
...
class PatDtorCompletion extends SymbolCompletion {
  edit (path :T.Path) {
    ...
    for (let aa = 0, arity = this.item.type.arity; aa < arity; aa += 1) {
...
class PatSpan extends SymRuleSpan {
  ...
  getCompletions (text :string) :M.Completion[] {
    ...
    const proto = TP.hole // this.tree.prototype
    console.log(`getCompletions for pat ${proto}`)

    // otherwise find a symbol that completes the text & matches the desired type
    const pred = (sym :S.Symbol) => (sym.kind === "term" && sym.flavor === "ctor" &&
                                     true /*TP.patApplies(sym.type(TP.hole, false), proto)*/)
    const comps :M.Completion[] = this.scope.getCompletions(pred, text).
  ...
  protected createDefaultComp (text :string) {
    return new PatBindCompletion(text, TP.hole/*this.tree.prototype*/) }
}
